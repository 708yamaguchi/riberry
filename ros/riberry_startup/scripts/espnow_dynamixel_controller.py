...
# Mainly copied from esp_now_pairing_single.py
...
def pairing_impl(com, role):
    ...
    if esp_now_pairing.role == Role.Main:
        esp_now_pairing.set_pairing_info("0.0.0.0")  # Forward (0.x.x.x)
        # esp_now_pairing.set_pairing_info("127.0.0.0")  # Stop (127.x.x.x)
        # esp_now_pairing.set_pairing_info("255.0.0.0")  # Reverse (255.x.x.x)
