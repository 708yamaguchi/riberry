...
def generate_spiral_stirring_trajectory(corners, corner_avs, step_width=0.02, lift_vector=None, stir_depth=0.0, manual_seed_av=None, **kwargs):
    ...
    for i in range(n_steps_spiral + 1):
        ...
        r_ratio = math.sqrt(t)  # 中心(0)から外(1.0)へ
        current_radius = max_radius * r_ratio
        current_angle = 2 * np.pi * spiral_turns * t
        add_point(current_radius, current_angle)

    # ==========================
    # Phase 2: 外周を1周
    # ==========================
    # 一番外側の半径を維持したまま、ぐるっと一周回る
    last_angle = 2 * np.pi * spiral_turns
    for i in range(1, n_steps_outer + 1):
        ratio = float(i) / n_steps_outer
        angle = last_angle + (2 * np.pi * ratio)
    ...
    # ==========================
    # 外側から中心へ戻る螺旋
    last_angle_after_outer = last_angle + (2 * np.pi)
    for i in range(1, n_steps_spiral + 1):
        t = float(i) / n_steps_spiral
        r_ratio = math.sqrt(1.0 - t)  # 外(1.0)から中心(0)へ
        current_radius = max_radius * r_ratio
        current_angle = last_angle_after_outer + (2 * np.pi * spiral_turns * t)
        add_point(current_radius, current_angle)
...
def generate_grid_pressing_trajectory(corners, corner_avs, step_width=0.05, lift_vector=None, press_stroke=0.03, manual_seed_av=None, press_steps=5, **kwargs):
    ...
    for i in range(n_rows + 1):
        ...
        for j in col_range:
            ...
            # --- 高さの補正 ---
            if press_stroke < 0:
                actual_stroke = abs(press_stroke)
                # スタート位置を基準面より「上」にする
                pos_top = p_safe_plane + (vec_up * actual_stroke)
                pos_bottom = p_safe_plane  # 基準面（底）まで下ろす
            else:
                pos_top = p_safe_plane     # 基準面（上）からスタート
                pos_bottom = p_safe_plane + (vec_down * press_stroke) # 下へ押し込む
...
class CornerTeachingTask:
    def __init__(self, ri, robot_model):
        ...
        self.ns = f"{self.ri.namespace}/" if self.ri.namespace else "/"
        ...
        self.const_params = {
            ...
            "gravity_comp_offset": 0.,   # [m] Vision認識時の重力補正高さ
            "lift_height": 0.1,            # [m] 移動時の持ち上げ高さ
            "stir_depth": 0.01,   # かき混ぜ深さ。正:基準面より深く、負:基準面より高い位置で動作 [m]
            "press_stroke": 0.15, # プレス量。正負に関わらず押し込み方向に動作。正:基準面から奥へ、負:空中から基準面へ [m]
            ...
            "ee_offset": (-0.12, 0.0, 0.25),  # ヘラをもつとき
            ...
            # "ee_offset": (-0.06, 0.0, 0.14),    # コロコロ用エンドエフェクタ
            # "ee_offset": (-0.05, 0.0, 0.25),    # ドライヤー用エンドエフェクタ
        ...
        raw_ns = self.ri.namespace.strip('/')
        # 例: "yamaguchi_arm_X/base_link"
        self.base_frame = f"{raw_ns}/base_link" if raw_ns else "base_link"
        camera_prefix = f"{raw_ns}_camera_" if raw_ns else "camera_"
        self.camera_frame = f"{camera_prefix}color_optical_frame"
        rospy.loginfo(f"TF Frames Initialized: Base='{self.base_frame}', Camera='{self.camera_frame}'")

        # --- ROS通信 ---
        self.pub_display = rospy.Publisher(self.ns + "atom_s3_additional_info", String, queue_size=1)
        rospy.Subscriber(self.ns + "atom_s3_mode", String, self._cb_atom_mode)
        rospy.Subscriber(self.ns + "atom_s3_button_state", Int32, self._cb_atom_button)
        self.atom_mode = ""
        self.current_button_state = 0

        self.servo_on_states = None
        servo_topic = self.ns + "fullbody_controller/servo_on_off_real_interface/state"
        rospy.Subscriber(servo_topic, ServoOnOff, self._cb_servo_on_states, queue_size=1)

        self.srv_server = rospy.Service(
            self.ns + 'task_instruction',
            TaskInstruction,
        ...
        rospy.Subscriber(self.ns + "imu", Imu, self._cb_imu)
    ...
    def teach_corners_vision(self, long_side_stroke=True):
        ...
        try:
            rospy.wait_for_service(self.ns + "estimate_corners", timeout=5.0)
            vision_srv = rospy.ServiceProxy(self.ns + "estimate_corners", VisualPose)
